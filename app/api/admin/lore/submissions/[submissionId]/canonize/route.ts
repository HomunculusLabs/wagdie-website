...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const submission = await loreSubmissionService.canonizeSubmission(await getSubmissionId(context), auth.address, readOptionalNote(body));
    return jsonOk(submission);
  } catch (error) {
    return handleLoreSubmissionApiError(error, 'Failed to canonize lore submission');
  }
}
